"""
增强版订单系统HTML（包含图片上传、自定义订单号、分批信息等）
"""
import os

def generate_enhanced_order_system_html():
    """生成增强版订单HTML代码"""
...
<body>
    ...
    <script>
        ...
        function loadData() {
            try {
                customers = JSON.parse(localStorage.getItem('cabinet_customers_v3')) || [];
                orders = JSON.parse(localStorage.getItem('cabinet_orders_v3')) || [];
        ...
        function initDemoData() {
            ...
            orders = [
                { id: 1, orderNo: "SAM001", type: "sample", customerId: 1, status: "pending", content: "客厅柜子打样", sentNo: "", estimatedDate: "2024-02-01", images: [], notes: "", createdAt: "2024-01-01T00:00:00.000Z" },
                { id: 2, orderNo: "DSN001", type: "design", customerId: 2, status: "processing", content: "厨房整体设计", estimatedDate: "2024-02-15", images: [], notes: "设计稿已确认", createdAt: "2024-01-02T00:00:00.000Z" },
                { id: 3, orderNo: "ORD001", type: "formal", customerId: 3, status: "completed", content: "全套橱柜定制", estimatedDate: "2024-03-01", isBatch: true, batchInfo: "分两批交付", batchDeliveryDate: "2024-03-01, 2024-03-15", images: [], notes: "等待确认", createdAt: "2024-01-03T00:00:00.000Z" },
                { id: 4, orderNo: "AFT001", type: "aftersale", customerId: 1, status: "processing", content: "维修服务", estimatedDate: "2024-01-20", images: [], notes: "上门维修", createdAt: "2024-01-04T00:00:00.000Z" }
            ];
            saveData();
        }
        ...
        function updateDashboard() {
            ...
            var html = recentOrders.length ? recentOrders.map(function(o) {
                ...
                return '<div class="card"><h3>' + o.orderNo + ' - ' + (typeNames[o.type] || o.type) + '</h3><p>客户: ' + (customer ? customer.code : '未知') + '</p><p>内容: ' + o.content + '</p><p>预计完成: ' + (o.estimatedDate || '-') + '</p><p>状态: <span class="status-badge status-' + o.status + '">' + getStatusText(o.status) + '</span></p></div>';
            }).join('') : '<p>暂无订单数据</p>';
        ...
        function submitOrder() {
            ...
            if (!orderNo || !content || !estimatedDate) { alert('请填写所有必填项！'); return; }
        ...
        function saveData() {
            localStorage.setItem('cabinet_customers_v3', JSON.stringify(customers));
            localStorage.setItem('cabinet_orders_v3', JSON.stringify(orders));
        }
...
if __name__ == "__main__":
    html_content = generate_enhanced_order_system_html()
    
    output_file = "/workspace/projects/assets/cabinet_system_v7_enhanced.html"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print("✅ 增强版订单系统HTML文件已生成：", output_file)
