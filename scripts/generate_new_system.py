"""
重新设计的橱柜订单管理系统 - 前端HTML
包含：客户管理、订单打样、设计报价、下单管理、售后管理
"""
import os
from coze_coding_dev_sdk.s3 import S3SyncStorage
from datetime import datetime

def generate_new_system_html():
    """生成新系统的HTML代码"""
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .upload-btn { display: inline-block; padding: 8px 20px; background: #667eea; color: white; border-radius: 5px; cursor: pointer; font-size: 14px; }
...
<body>
    <div class="container">
        <header>
            <h1>🏠 橱柜订单管理系统</h1>
            <p>数据保存在浏览器本地 | 客户Logo支持上传</p>
        </div>
        <div class="nav">
            <button class="active" onclick="showPage('dashboard')">📊 仪表盘</button>
            <button onclick="showPage('customers')">👥 客户管理</button>
            <button onclick="showPage('orders_sample')">🎨 订单打样</button>
            <button onclick="showPage('orders_design')">💰 设计报价</button>
            <button onclick="showPage('orders_formal')">📝 下单管理</button>
            <button onclick="showPage('orders_aftersale')">🔧 售后管理</button>
        </div>
        ...
        <div id="dashboard" class="page active">
            <div class="stats">
                ...
                <div class="stat-card"><h3 id="totalSampleOrders">0</h3><p>打样订单</p></div>
                <div class="stat-card"><h3 id="totalDesignOrders">0</h3><p>设计报价</p></div>
                <div class="stat-card"><h3 id="totalFormalOrders">0</h3><p>正式订单</p></div>
                <div class="stat-card"><h3 id="totalAfterSaleOrders">0</h3><p>售后订单</p></div>
            </div>
            <h2>📈 最近订单</h2>
    ...
    <script>
        // 数据存储
        var customers = JSON.parse(localStorage.getItem('cabinet_customers_v2')) || [];
        var orders = JSON.parse(localStorage.getItem('cabinet_orders_v2')) || [];
        ...
        function initDemoData() {
            ...
            orders = [
                { id: 1, orderNo: "SAM202401010001", type: "sample", customerId: 1, status: "processing", content: "客厅柜子打样", amount: 0, notes: "", createdAt: "2024-01-01T00:00:00.000Z" },
                { id: 2, orderNo: "DSN202401020001", type: "design", customerId: 2, status: "completed", content: "厨房整体设计", amount: 35000, notes: "设计稿已确认", createdAt: "2024-01-02T00:00:00.000Z" },
                { id: 3, orderNo: "ORD202401030001", type: "formal", customerId: 3, status: "pending", content: "全套橱柜定制", amount: 85000, notes: "等待确认", createdAt: "2024-01-03T00:00:00.000Z" },
                { id: 4, orderNo: "AFT202401040001", type: "aftersale", customerId: 1, status: "processing", content: "维修服务", amount: 1200, notes: "上门维修", createdAt: "2024-01-04T00:00:00.000Z" }
            ];
            saveData();
        }
        ...
        function updateDashboard() {
            ...
            var html = recentOrders.length ? recentOrders.map(function(o) {
                ...
                return '<div class="card"><h3>' + o.orderNo + ' - ' + (typeNames[o.type] || o.type) + '</h3><p>客户: ' + (customer ? customer.code : '未知') + '</p><p>内容: ' + o.content + '</p><p>金额: ' + (o.amount > 0 ? '¥' + o.amount.toLocaleString() : '-') + '</p><p>状态: <span class="status-badge status-' + o.status + '">' + getStatusText(o.status) + '</span></p></div>';
            }).join('') : '<p>暂无订单数据</p>';
        ...
        function showAddOrderForm(type) {
            ...
            editingOrderId = null;
            var typePrefixes = { sample: 'SAM', design: 'DSN', formal: 'ORD', aftersale: 'AFT' };
            ...
            html += '<div class="form-group"><label>金额（元）</label><input type="number" id="orderAmount" min="0" step="100" value="0"></div>';
        ...
        function submitOrder() {
            ...
            var typePrefixes = { sample: 'SAM', design: 'DSN', formal: 'ORD', aftersale: 'AFT' };
            var prefix = typePrefixes[currentOrderType];
            var dateStr = new Date().toISOString().slice(0,10).replace(/-/g, '');
            var seq = String(orders.filter(function(o) { return o.type === currentOrderType; }).length + 1).padStart(4, '0');
            
            orders.push({
                id: Date.now(),
                orderNo: prefix + dateStr + seq,
        ...
        function filterOrders(type) {
            ...
            var filtered = typeOrders.filter(function(o) { 
                var customer = customers.find(function(c) { return c.id === o.customerId; });
                return o.orderNo.toLowerCase().includes(search) || o.content.toLowerCase().includes(search) || (customer && customer.code.toLowerCase().includes(search));
            });
        ...
        function saveData() {
            localStorage.setItem('cabinet_customers_v2', JSON.stringify(customers));
            localStorage.setItem('cabinet_orders_v2', JSON.stringify(orders));
        }
...
if __name__ == "__main__":
    html_content = generate_new_system_html()
    
    # 保存到文件
    output_file = "/workspace/projects/assets/cabinet_system_v2.html"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print("✅ 新系统HTML文件已生成：", output_file)
