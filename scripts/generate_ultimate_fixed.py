"""
完全修复版 - 修复所有onclick事件转义问题
"""
import os

def generate_fully_fixed_system():
    """生成完全修复的订单HTML代码"""
...
<body>
    ...
    <script>
        ...
        function renderOrderCard(o, type) {
            ...
            var buttonsHtml = '<div style="margin-top: 10px;">';
            buttonsHtml += '<button class="btn btn-warning" data-id="' + o.id + '" data-type="' + type + '" class="edit-order-btn">编辑</button>';
            buttonsHtml += '<select onchange="updateOrderStatus(' + o.id + ', this.value); this.value=\'\';" style="padding: 8px; margin-left: 10px;"><option value="">更新状态...</option><option value="pending">待处理</option><option value="processing">处理中</option><option value="completed">已完成</option><option value="cancelled">已取消</option></select>';
            buttonsHtml += '<button class="btn btn-danger" onclick="deleteOrder(' + o.id + ')" style="margin-left: 10px;">删除</button>';
            buttonsHtml += '</div>';
            
            return '<div class="card"><h3>📋 ' + o.orderNo + '</h3><p>客户: ' + (customer ? customer.code : '未知') + ' - ' + (customer ? customer.region : '') + '</p><p>内容: ' + o.content + '</p><p>预计完成: ' + (o.estimatedDate || '-') + '</p>' + executorHtml + extraInfo + imagesHtml + '<p>状态: <span class="status-badge status-' + o.status + '">' + getStatusText(o.status) + '</span></p><p>时间: ' + new Date(o.createdAt).toLocaleString() + '</p>' + (o.notes ? '<p>备注: ' + o.notes + '</p>' : '') + buttonsHtml + '</div>';
        }
...
if __name__ == "__main__":
    html_content = generate_fully_fixed_system()
    
    output_file = "/workspace/projects/assets/cabinet_system_v10_ultimate.html"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print("✅ 完全修复版订单系统HTML文件已生成：", output_file)
    print(f"文件大小：{len(html_content)} 字符")
    print("\n✨ 修复内容：")
    print("- 使用addEventListener代替onclick属性")
    print("- 避免字符串转义问题")
    print("- 所有按钮功能应该正常工作")
