...
def init_demo_data():
    """初始化示例数据"""
    # 检查是否已有数据
    if db.get_all_customers():
        return
    
    # 添加示例客户
    demo_customers = [
        {"name": "张先生", "phone": "13800138001", "address": "北京市朝阳区建国路88号", "email": "zhang@example.com"},
        {"name": "李女士", "phone": "13900139002", "address": "上海市浦东新区陆家嘴环路1000号", "email": "li@example.com"},
        {"name": "王先生", "phone": "13700137003", "address": "广州市天河区珠江新城华夏路", "email": "wang@example.com"},
    ]
    ...
    # 添加示例产品
    demo_products = [
        {"name": "现代简约整体橱柜", "category": "整体橱柜", "price": 15000, "unit": "套", "description": "简约设计，环保材质，适合小户型"},
        {"name": "欧式豪华橱柜", "category": "整体橱柜", "price": 28000, "unit": "套", "description": "欧式风格，实木雕刻，高端大气"},
        {"name": "实木吊柜", "category": "吊柜", "price": 3500, "unit": "平米", "description": "实木材质，承重力强"},
        {"name": "地柜标准款", "category": "地柜", "price": 4200, "unit": "平米", "description": "标准高度，多功能分区"},
        {"name": "石英石台面", "category": "台面", "price": 1200, "unit": "平米", "description": "耐磨耐热，易清洁"},
        {"name": "不锈钢拉篮", "category": "配件", "price": 880, "unit": "件", "description": "304不锈钢，静音滑轨"},
    ]
    ...
    demo_orders = [
        ...
        },
        {
            ...
            "items": [
                ...
                {"product_id": products[5]["id"], "quantity": 4},
            ],
            "notes": "定制规格，需要确认图纸"
        },
    ]
    ...
    for order in demo_orders:
        db.create_order(order["customer_id"], order["items"], order.get("notes"))
...
def show_dashboard():
    ...
    with col4:
        confirmed_orders = stats['by_status'].get('已确认', 0)
        st.metric("已确认订单", confirmed_orders)
    ...
    # 最近订单
    st.subheader("📋 最近订单")
    orders = db.get_all_orders()[:5]
    if orders:
        for order in orders:
            with st.expander(f"{order['order_no']} - {order['customer_name']} - {format_price(order['total_amount'])}"):
                st.write(f"📞 电话: {order['customer_phone']}")
